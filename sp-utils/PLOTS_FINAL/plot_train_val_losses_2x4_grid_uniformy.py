...
def main():
    ...
    # ---- Compute global y-limits across all selected logs (uniform y-axis) ----
    global_min = float('inf')
    global_max = float('-inf')
    parsed_cache = {}  # ns -> (epochs, tr, va)

    for ns, path in items:
        # Parse once and cache; also use values to compute global y-limits
        epochs, tr, va = parse_log(path, max_loss=args.max_loss, max_epoch=args.max_epoch)
        parsed_cache[ns] = (epochs, tr, va)
        if epochs:
            local_min = min(min(tr), min(va))
            local_max = max(max(tr), max(va))
