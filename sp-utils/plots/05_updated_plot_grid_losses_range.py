...
def parse_args():
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Optional: include only nsteps >= this value (default: no lower bound)",
    )
    p.add_argument(
        "--nstep_max",
        type=int,
        default=None,
        help="Optional: include only nsteps <= this value (default: no upper bound)",
    )
    p.add_argument(
        "--output",
        default=None,
...
def main():
    ...
    # Optional range filter on nsteps (applied after parity filter)
    if args.nstep_min is not None:
        nsteps = [n for n in nsteps if n >= args.nstep_min]
    if args.nstep_max is not None:
        nsteps = [n for n in nsteps if n <= args.nstep_max]
